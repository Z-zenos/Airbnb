  ...
  async function handleSubmitForm(ev) {
    ...
    if(formData.password !== formData.passwordConfirm) {
      ...
      try {
        ...
        const res = await axios.post('http://localhost:3000/api/v1/auth/signup', body, config);
        ...
        <div className="p-6 relative">
          ...
          <form className="w-2/3" onSubmit={handleSubmit(handleSubmitForm)}>
            <div className="relative">
              {/* FOR LOGIN */}
              <Input label="Email" name="email" type="email" className="rounded-t-[8px]" value={email} onChange={handleFormDataChange} />

              {errors.email && (
                <p>{errors.email.message}</p>
              )}
              <Input label="Password" name="password" type="password" className="border-t-transparent" value={password} onChange={handleFormDataChange} />

              {/* FOR REGISTER */}
              <Input label="Confirm password" name="passwordConfirm" type="password" className="border-t-transparent" value={passwordConfirm} onChange={handleFormDataChange} />
              <Input label="Name" type="text" name="name" className="border-t-transparent" value={name} onChange={handleFormDataChange} />
              ...
              <Input label="Phone number" beforeText={countries.length && countries?.find(c => c.name === country).dialling_code} name="phone" type="tel" className="rounded-b-[8px] border-t-transparent" value={phone} onChange={handleFormDataChange} />
