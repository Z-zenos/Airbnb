    ...
    <div className={`w-[270px] my-6 cursor-pointer ${className}`}>
      <div className="relative rounded-lg w-full h-[260px]">
        <div className="absolute right-4 top-4 z-[1]">
          { heart 
            ? <FaHeart className={`w-5 h-5 text-primary transition-all active:scale-100`} onClick={async () => await handleUpdateWishlists()} />
            : <FaRegHeart className={`w-5 h-5 hover:scale-110 transition-all active:scale-80`} onClick={async () => await handleUpdateWishlists()} />
          }
        </div>
